...
Since the last attempt was so successful, I've decided to publish the scripts for it. I've went through them and added comments in the code to explain the inner workings. Hopefully they clarify most of the things! In-depth analysis and explanation of the solutions developed for these scripts will be provided in an upcoming article, which should cover both the current repo and this one:
...
*   `C2.txt` - contains the exact timing for C2. The format must be compatible with `date` utility!
*   `C3.txt` \- contains the exact timing for C3. The format must be compatible with `date` utility!
...
*   `SIM_C2.txt` - contains the simulation timing for C2. The format must be compatible with `date` utility!
*   `SIM_C3.txt` \- contains the simulation timing for C3. The format must be compatible with `date` utility!

To run the scripts please ensure that your system has `bash`, `sed` and `awk` (I am not listing the full list because the rest of them are pretty much standard utilities for any Unix-alike OS).
